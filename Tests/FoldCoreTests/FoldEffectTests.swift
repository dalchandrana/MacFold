...
@Test func duoIsTheDefaultAndUnknownSelectionsFallBackToIt() {
    ...
    #expect(FoldEffect.resolve(shaderIndex:5) == .duo)
    #expect(FoldEffect.resolve(shaderIndex:.max) == .duo)
}

@Test func persistedIdentifiersAndShaderIndicesAreStable() {
    #expect(FoldEffect.allCases.map(\.persistedIdentifier) == ["duo","roll","shutter","flex","iris"])
    #expect(FoldEffect.allCases.map(\.shaderIndex) == [0,1,2,3,4])
...
@Test func everyEffectIsPresentableAndOnlyDuoKeepsTheSharpPath() {
    #expect(FoldEffect.allCases.count == 5)
    for effect in FoldEffect.allCases {
